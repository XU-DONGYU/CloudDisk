...
static std::uniform_int_distribution<int> dist{ 0, 61 };   /* alpha的长度为62 */
...
std::string CryptoUtil::generate_token(const std::string& username)
{
    jwt_t* jwt;
    jwt_new(&jwt);  // 创建 JWT
    ...
    jwt_add_grant(jwt, "username", username.c_str());	// 用户ID
...
bool CryptoUtil::verify_token(const std::string& token, std::string& username)
{
    jwt_t* jwt;
    ...
    username = jwt_get_grant(jwt, "username");
    return true;
}
